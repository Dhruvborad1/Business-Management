...
//   // Drawer / Bottom Sheet States for Mobile & Tablet (< 1280px / xl)
//   const [isDrawerOpen, setIsDrawerOpen] = useState(false)
//   const [searchQuery, setSearchQuery] = useState('')

//   const dragControls = useDragControls()

//   // Common Tailwind Scrollbar Utility Class String
...
//   // Filtered party groups for drawer search bar
...
//         const matchingChalans = group.chalans.filter(
//           (c) =>
//             c.chalanNumber?.toString().toLowerCase().includes(query) ||
//             group.partyName.toLowerCase().includes(query)
//         )

//         if (matchesParty || matchingChalans.length > 0) {
//           return {
//             ...group,
//             chalans: matchesParty ? group.chalans : matchingChalans,
//           }
//         }
//         return null
//       })
...
//   const handleSelectChalanMobile = (chalan, partyName) => {
//     setSelectedPartyName(partyName)
//     setSelectedId(chalan.id)
...
//               {group.chalans.length} challan{group.chalans.length > 1 ? 's' : ''}
//             </span>
//           </div>
...
//                     onClick={() =>
//                       isDrawer
//                         ? handleSelectChalanMobile(chalan, group.partyName)
//                         : setSelectedId(chalan.id)
//                     }
...
//                         {formatDisplayDate(chalan.chalanDate)}
//                       </span>
//                     </div>
//                   </button>
//                 ))}
...
//         {/* Left Side Sidebar - Direct Tailwind Custom Scrollbar */}
...
//         {/* Right Side Preview Container - Direct Tailwind Custom Scrollbar */}
//         <div className="flex h-[calc(100vh-4rem)] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
...
//           <div className={`flex-1 overflow-y-auto pr-1 ${customScrollbarClass}`}>
//             {selectedChalan ? (
...
//               <div className="flex items-center gap-2">
//                 <span className="text-sm font-semibold text-slate-900">Your Challans</span>
...
//       {/* Floating Bottom Sheet / Drawer Drawer (< 1280px / xl) - Direct Tailwind Custom Scrollbar */}
//       <AnimatePresence>
//         {isDrawerOpen && (
//           <>
//             <motion.div
...
//                     <h3 className="text-base font-semibold text-slate-900">Your Challans</h3>
...
//                     placeholder="Search party or chalan..."
...
//               {/* Party & Challan Accordion List with Direct Tailwind Scrollbar */}
...
function YourChalanReport({ yourChalans, setYourChalans }) {
  ...
  // Ref for auto-calculating preview scale on mobile
  ...
  // Auto calculate scale for small screen width
  useEffect(() => {
    const handleAutoFitScale = () => {
      if (previewContainerRef.current) {
        const containerWidth = previewContainerRef.current.clientWidth - 24 // Padding account
        const standardChalanWidth = 794 // Standard A4 / Chalan width

        if (containerWidth < standardChalanWidth) {
          const calculatedScale = Number((containerWidth / standardChalanWidth).toFixed(2))
              ...
              <div className="space-y-2">
                {group.chalans.map((chalan) => (
                  <button
                    ...
                    className={`w-full rounded-xl border px-3 py-2 text-left text-sm transition ${selectedChalan?.id === chalan.id
                        ? 'border-violet-200 bg-violet-50 text-violet-900 font-medium'
                        : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'
                      }`}
                  >
